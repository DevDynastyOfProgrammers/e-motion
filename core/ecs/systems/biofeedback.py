...
class BiofeedbackWorker(threading.Thread):
    ...
    def _run_vision_inference(self, frame: Optional[np.ndarray]) -> EmotionPrediction:
    ...
    def _prepare_debug_frame(self, frame: Optional[np.ndarray]) -> Optional[np.ndarray]:
        if frame is None:
            return None
        try:
...
class BiofeedbackSystem:
    ...
    def __init__(self, event_manager: EventManager, director: GameDirector):
        ...
        self.current_debug_frame: Optional[np.ndarray] = None
